...
```
Narrow down the predictors, fit full model
```{r}
# chose all numerical vars (corr > 20% w logmsrp) and top 4 cat. vars (corr > 25% w logmsrp)
predictors <- cars %>% select(logmsrp, popularity, year, engine_hp, highway_mpg, city_mpg, engine_cylinders, engine_fuel_type, vehicle_size, transmission_type)

fit.all <- lm(logmsrp~., data=predictors)
ols_regress(fit.all) # view model summary, parameter estimates
ols_vif_tol(fit.all) # view vif. a couple are concerning
```
Lasso
```{r}
...
Forward selection
```{r}
forward <- regsubsets(logmsrp~popularity + year + engine_hp + highway_mpg + city_mpg + engine_cylinders + engine_fuel_type + vehicle_size + transmission_type, data=cars, method="forward", nvmax=10)

summary(forward)$adjr2
summary(forward)$bic
summary(forward)$rss

forward.train <- regsubsets(logmsrp~popularity + year + engine_hp + highway_mpg + city_mpg + engine_cylinders + engine_fuel_type + vehicle_size + transmission_type, data=train, method="forward", nvmax=10)
...
# optimal predictors based on ase. 7 is best
testASE<-c()
for (i in 1:10){
    predictions<-predict.regsubsets(object=forward.train,newdata=test,id=i) 
    testASE[i]<-mean((log(test$logmsrp)-predictions)^2)
}
par(mfrow=c(1,1))
plot(1:10,testASE,type="l",xlab="# of predictors",ylab="test vs train ASE",ylim=c(10, 80))
index<-which(testASE==min(testASE))
points(index,testASE[index],col="red",pch=10)
rss<-summary(reg.fwd)$rss
lines(1:10,rss/9824,lty=3,col="blue")

forward.final <- regsubsets(logmsrp~popularity + year + engine_hp + highway_mpg + city_mpg + engine_cylinders + engine_fuel_type + vehicle_size + transmission_type, data=cars, method="forward", nvmax=7)

coef(forward.final,7)
forward.final <-lm(logmsrp~year+engine_hp+vehicle_size+engine_fuel_type,data=cars)
