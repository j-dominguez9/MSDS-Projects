...
```
msrp vs. make & msrp vs. vehicle_style (did not plot msrp vs. model bc unreadable at 65 levels)
```{r}
# msrp vs. make
...
<<<<<<< HEAD
p9 <- cars %>% ggplot(aes(x=engine_cylinders, y=msrp)) + geom_boxplot()

## msrp vs. engine_fuel_type
p10 <- cars %>% ggplot(aes(x=engine_fuel_type, y=msrp)) + geom_boxplot()

## msrp vs. driven_wheels
p11 <- cars %>% ggplot(aes(x=driven_wheels, y=msrp)) + geom_boxplot()
...
```{r}
>>>>>>> c31c57247c54a3314f06a54d6c0cffc3a34ca60c
...
<<<<<<< HEAD
p12 <- cars %>% ggplot(aes(y = msrp, fill = transmission_type))+geom_boxplot()
=======
```
msrp vs. driven_wheels
```{r}
...
```{r}
vs_levels <- cars %>% select(msrp, vehicle_style) %>%
  group_by(vehicle_style) %>% summarize(median(msrp)) %>% arrange((`median(msrp)`))
>>>>>>> c31c57247c54a3314f06a54d6c0cffc3a34ca60c

## msrp vs. vehicle_size
p13 <- cars %>% ggplot(aes(x=vehicle_size, y=msrp)) + geom_boxplot()

## msrp vs. number_of_doors
p14 <- cars %>% ggplot(aes(x=number_of_doors, y=msrp)) + geom_boxplot()
