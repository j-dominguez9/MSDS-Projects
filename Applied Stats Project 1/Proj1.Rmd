...
```
logmsrp correlation with numerical variables
```{r}
library(GGally)
library(ggpairs)
logcars <- cars %>% mutate(logmsrp = log(msrp), logengine_hp = log(engine_hp), loghighway_mpg = log(highway_mpg), logcity_mpg = log(city_mpg))

# logmsrp vs numerical vars, some logged
# include all num var in model
# highway and city mpg have slightly higher corr with logmsrp when logged
ggpairs(logcars, columns = c('logmsrp', 'year', 'popularity', 'logengine_hp', 'loghighway_mpg', 'logcity_mpg'))

ggpairs(logcars, columns = c('logmsrp', 'year', 'popularity', 'engine_hp', 'highway_mpg', 'city_mpg'))
```
logmsrp with categorical variables
```{r warning=FALSE}
hetcor(logcars$logmsrp, logcars$engine_fuel_type, logcars$engine_cylinders, logcars$market_category, logcars$vehicle_size, logcars$make, logcars$model, logcars$transmission_type, logcars$driven_wheels, logcars$number_of_doors, logcars$vehicle_style)
# caution: takes a few mins to run. here is output to save you time in desc order
# engine_cylinders : .5578
# engine_fuel_type : -.4082
# vehicle_size : .2766
# transmission_type : -.2544
# make : -.2271
...
Fit the model
```{r}
fit.simple <- lm(logmsrp~year+popularity+engine_hp+highway_mpg+city_mpg+engine_cylinders + engine_fuel_type + vehicle_size + transmission_type, data=logcars)
