...
#model 3 with all variables and unlogged msrp
tree3 <- rpart(msrp ~., data = train, method = 'anova',
                    control = rpart.control(minbucket = 100))
...
#model 4 with all variables except model and unlogged msrp 
tree4 <- rpart(msrp ~ engine_hp + market_category + make + engine_cylinders + vehicle_size + engine_fuel_type + transmission_type + vehicle_style + city_mpg + highway_mpg, data = train, method = 'anova',
                    control = rpart.control(minbucket = 100))
...
#model 5 with caret all variables except model - performs but less risk of overfitting
trctrl <- trainControl(method = 'repeatedcv', repeats = 3, number = 10)
tree5 <- train(msrp ~ engine_hp + market_category + make + engine_cylinders + vehicle_size + engine_fuel_type + transmission_type + vehicle_style + city_mpg + highway_mpg + make, data = train, method = "rpart2")

treep5 <- predict(tree5, test)

ASE(test$msrp, treep5)

sqrt(ASE(test$msrp, treep5))

rpart.plot(tree5$finalModel, type = 1, digits = 3, fallen.leaves = TRUE)
...
p5 <- test2 %>% ggplot(aes(x = predicted5, y = msrp)) + geom_point() + geom_smooth(method = 'lm')
library(cowplot)
...
summarize_tree(tree5$finalModel)
```
...
# Nonparametric model
np.train <- tree4
np.test <- predict(tree4, test)
metrics_galore(test$msrp, np.test)
...
# Nonparametric model
np.valid <- predict(tree4, valid)
metrics_galore(valid$msrp, np.valid)
```
