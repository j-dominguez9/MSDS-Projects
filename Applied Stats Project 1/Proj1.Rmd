...
```
set relevant variables as factors. Popularity as factor bc each make shares the same popularity score so it's not really continuous
```{r}
...
summary(cars)
```
Distribution of msrp. possible log transformation prior to analysis
```{r}
...
# boxplot of msrp vs market_category
...
msrp vs. city_mpg
```{r}
cars %>% ggplot(aes(x=highway_mpg, y=msrp)) + geom_point()
```
msrp vs year
```{r}
