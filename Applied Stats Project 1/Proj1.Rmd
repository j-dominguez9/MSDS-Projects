...
```{r}
numvars <- cars %>% select(logmsrp, popularity, year, engine_hp, highway_mpg, city_mpg)
cor.numvars <- cor(numvars)

# correlogram. use library(corrplot)
# high correlation = blue color and larger size
corrplot(cor.numvars, type = "upper", order = "hclust", tl.col = "black", tl.srt = 45)

# correlation chart. use library(GGally)
ggpairs(cars, columns = colnames(numvars))
...
```
log msrp; EDA / correlation with *categorical* variables
```{r warning=FALSE}
...
# did not plot make, model, market_category bc insanely high # of levels
pairs <- ggpairs(data=cars, columns = c('logmsrp', 'engine_cylinders', 'engine_fuel_type', 'vehicle_size', 'transmission_type'))
...
pairs2 <- ggpairs(data=cars, columns = c('logmsrp', 'driven_wheels', 'number_of_doors', 'vehicle_style'), cardinality_threshold = 16)
plots <- lapply(1:pairs2$ncol, function(j) getPlot(pairs2, i = 1, j = j))
ggmatrix(
    plots,
    nrow = 1,
    ncol = pairs$ncol,
    xAxisLabels = pairs2$xAxisLabels,
    yAxisLabels = pairs2$yAxisLabels
)
```
Narrow down the predictors
```{r}
fit.simple <- lm(logmsrp~year+popularity+engine_hp+highway_mpg+city_mpg+engine_cylinders + engine_fuel_type + vehicle_size + transmission_type, data=cars)
ols_regress(fit.simple) # view model summary, parameter estimates
ols_vif_tol(fit.simple) # to view VIF. nothing too concerning except electric (engine_fuel_type) & direct_drive (transmission_type)

# fit.simple: adjr2 .797 rmse=0.203 mae=0.153 interesting that engine_cylinders has high corr with logmsrp yet insig here
# the full model with all predictors and unlogged msrp : adj.r2=.968 rmse=3656 mae=2407
# full model with all predictors and logmsrp : adjr2 .966 rmse=.083 mae=.062
```
Lasso
```{r}
x=model.matrix(logmsrp~.-msrp,train)[,-2]
y=log(train$logmsrp)
xtest<-model.matrix(logmsrp~.-msrp,test)[,-1]
...
Forward selection
```{r}
selection.fwd <- regsubsets(logmsrp~.-msrp, data=cars, method="forward", nvmax=10)
coef(selection.fwd,10)
summary(selection.fwd)$adjr2
summary(selection.fwd)$bic
summary(selection.fwd)$rss
```
Fit the model
```{r}
...
Assumptions check
```{r}
ols_plot_diagnostics(fit.simple, print_plot = FALSE) # view residual plots
# residual vs predicted plot looks good, couple obs veer away at upper margin
# residual histogram looks great
# residual qqplot: veers off a bit at the higher end
# cook's d looks great; no large influential obs
# outlier & leverage plot: few obs with outliers, leverage; not unexpected with 10k obs
```
# Complex Model
```{r}
